# FUSE Documentation Index

FUSE, the Flexible User-extensible Simulation Editor, is a plugin-oriented graphical application for building, configuring, validating, and eventually exporting simulation models.

This documentation is organized by audience and topic.

## User documentation

- [Quickstart](user/quickstart.md)
- [User guide](user/user-guide.md)
- [Model editor workflow](user/model-editor-workflow.md)
- [Composite components](user/composite-components.md)
- [Properties and validation](user/properties-and-validation.md)
- [Project files and save/load](user/project-files.md)
- [Troubleshooting](user/troubleshooting.md)

## Developer documentation

- [v0.7.0 release notes draft](releases/v0.7.0.md)
- [v0.6.0 release notes draft](releases/v0.6.0.md)
- [Development guide](developer/development-guide.md)
- [Codebase tour](developer/codebase-tour.md)
- [Resource and icon guide](developer/resources-and-icons.md)
- [Release process](developer/release-process.md)
- [Maintenance checklist](developer/maintenance-checklist.md)

## Architecture documentation

- [Architecture overview](architecture/architecture-overview.md)
- [Application startup](architecture/application-startup.md)
- [Data model](architecture/data-model.md)
- [Persistence and database](architecture/persistence-and-database.md)
- [Routing and canvas graphics](architecture/routing-and-canvas.md)

## Plugin documentation

- [Plugin system](plugins/plugin-system.md)
- [Plugin development guide](plugins/plugin-development-guide.md)
- [SST plugin](plugins/sst/sst-plugin.md)
- [SST database schema](plugins/sst/sst-database-schema.md)
- [SST import workflow](plugins/sst/sst-import-workflow.md)
- [SST external validation harness](plugins/sst/sst-external-validation.md)
- [gem5 plugin](plugins/gem5/gem5-plugin.md)

## Testing and CI

- [Testing guide](testing/testing-guide.md)
- [Test matrix](testing/test-matrix.md)
- [GitHub Actions CI/CD](ci/github-actions.md)
- [SST CI container](ci/sst-ci-container.md)
- [gem5 CI container](ci/gem5-ci-container.md)

## Reference

- [Command reference](reference/commands.md)
- [Environment variables](reference/environment-variables.md)
- [FUSE project file format](reference/fse-format.md)
- [Component metadata reference](reference/component-metadata.md)
- [Plugin manifest reference](reference/plugin-manifest.md)
- [Plugin API reference](reference/plugin-api.md)
- [Database table reference](reference/database-tables.md)
- [Glossary](reference/glossary.md)

## Examples

- [Example: build a simple model](examples/simple-model.md)
- [Example: import SST metadata](examples/import-sst-metadata.md)
- [Example: write a simple plugin](examples/write-a-plugin.md)
- [Example: component definitions](examples/component-definitions.md)
- [Example: troubleshoot validation issues](examples/validation-walkthrough.md)

- [SST JSON Export Guide](plugins/sst/sst-json-export-guide.md)
